...
with r1c2:
    ...
    query = 'SELECT YEAR(date) as year, company, MAX(high) AS Highest FROM dashboard.stockprice WHERE YEAR(date) in (2021, 2022, 2023) GROUP BY company, year ORDER BY Highest DESC;'
...
with r3c1:
    ...
    df_fn2 = (filtered_df_fn.groupby('publisher').size().reset_index(name='Total'))
    df_fn2 = (df_fn2.sort_values(by="Total", ascending=False)).head(10)
    st.dataframe(df_fn2,
                 ...
                 column_config={
                     ...
                     "Total": st.column_config.ProgressColumn("Total",format="%f",min_value=0,max_value=max(df_fn2.Total),)
                     }
                 )
...
with r3c2:
    ...
    df_fn2 = filtered_df_fn.groupby('publisher').size().reset_index(name='Total')
    fig = px.bar(df_fn2,x='Total', y='publisher', template='seaborn')
    fig.update_traces(text=df_fn2['publisher'], textposition='inside')
